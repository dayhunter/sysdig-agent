...
function create_sysdig_serviceaccount {
    ...
    out=$(kubectl create serviceaccount sysdig-agent-patis --namespace=$NAMESPACE 2>&1) || { fail=1 && echo "kubectl create serviceaccount failed!"; }
    if [ $fail -eq 1 ]; then
        if [[ "$out" =~ "AlreadyExists" ]]; then
            echo "$out. Continuing..."
        else
...
function install_k8s_agent {
    ...
    fail=0
    outbinding=$(kubectl create clusterrolebinding sysdig-agent-patis --clusterrole=sysdig-agent --serviceaccount=$NAMESPACE:sysdig-agent-patis --namespace=$NAMESPACE 2>&1) || { fail=1 && echo "kubectl create clusterrolebinding failed!"; }
    if [ $fail -eq 1 ]; then
    ...
    fail=0
    outsecret=$(kubectl create secret generic sysdig-agent-patis --from-literal=access-key=$ACCESS_KEY --namespace=$NAMESPACE 2>&1) || { fail=1 && echo "kubectl create secret failed!"; }
    if [ $fail -eq 1 ]; then
        if [[ "$outsecret" =~ "AlreadyExists" ]]; then
            echo "$outsecret. Re-creating secret..."
            kubectl delete secrets sysdig-agent-patis --namespace=$NAMESPACE 2>&1
            kubectl create secret generic sysdig-agent-patis --from-literal=access-key=$ACCESS_KEY --namespace=$NAMESPACE 2>&1
        else
            echo "$outsecret"
            exit 1
        fi
    ...
    sed -i -e "s|# serviceAccount: sysdig-agent-patis|serviceAccount: sysdig-agent-patis|" /tmp/sysdig-agent-daemonset-v2.yaml
...
function remove_agent {
    ...
    kubectl delete daemonset sysdig-agent-patis --namespace=$NAMESPACE

    echo "* Deleting the sysdig-agent configmap"
    kubectl delete configmap sysdig-agent-patis --namespace=$NAMESPACE

    echo "* Deleting the sysdig-agent serviceacccount"
    kubectl delete serviceaccount -n default sysdig-agent-patis --namespace=$NAMESPACE

    echo "* deleting the sysdig-agent clusterrolebinding"
    kubectl delete clusterrolebinding sysdig-agent-patis --namespace=$NAMESPACE

    echo "* Deleting the sysdig-agent secret"
    kubectl delete secret sysdig-agent-patis --namespace=$NAMESPACE
